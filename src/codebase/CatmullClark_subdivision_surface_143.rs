// Rosetta Code task: Catmull–Clark subdivision surface
// Source: https://rosettacode.org/wiki/Catmull–Clark_subdivision_surface#Rust
// Content licensed under GFDL 1.2 (Rosetta Code).
// =======================
// Expected output:
// Before: 8 vertices, 12 faces
// After 1 subdivision: 38 vertices, 72 faces
// v[0] = (-0.7222, -0.7222, -0.7222)
// v[1] = (-0.5833, -0.5833, 0.7917)
// v[2] = (-0.5833, 0.7917, -0.5833)
// v[3] = (-0.7917, 0.5833, 0.5833)
// v[4] = (0.7917, -0.5833, -0.5833)
// =======================
...
pub struct Vector2 { pub u: f64, pub v: f64 }
impl Vector2 {
    pub fn new(u: f64, v: f64) -> Vector2 { Vector2 { u, v } }
    pub fn copy(&self) -> Vector2 { Vector2 { u: self.u, v: self.v } }
}

// ---- Missing Vector3 operators ----
// Add: Vector3 + &Vector3  (used as p4 + &p3, line_start.copy() + &line_i)
...
// Sub: Vector3 - &Vector3  (line_end.copy() - line_start)
...
// Mul by scalar: Vector3 * f64  (old_coords * m1, line * t)
...
// Div by scalar: Vector3 / f64  (tp / div)
...
// AddAssign &Vector3  (face_point += &curr_point)
impl AddAssign<&Vector3> for Vector3 {
    fn add_assign(&mut self, rhs: &Vector3) {
        self.x += rhs.x; self.y += rhs.y; self.z += rhs.z;
    }
}
// DivAssign f64  (face_point /= len)
...
// ---- Missing Mesh::center_point ----
impl Mesh {
    pub fn center_point(p1: &Vector3, p2: &Vector3) -> Vector3 {
        Vector3::new(
            (p1.x + p2.x) / 2.0,
            (p1.y + p2.y) / 2.0,
            (p1.z + p2.z) / 2.0,
...
fn main() {
    // A unit cube as 12 triangles (8 vertices).
    let verts = vec![
    ...
    // Triangle index triples (two per cube face).
    let tris = [
    ...
    let mut mesh = Mesh { v: verts, f: faces };

    println!("Before: {} vertices, {} faces", mesh.v.len(), mesh.f.len());
    mesh.subdivide();
    println!("After 1 subdivision: {} vertices, {} faces", mesh.v.len(), mesh.f.len());

    // Print a few resulting vertex coordinates so output is visible.
    for (i, v) in mesh.v.iter().take(5).enumerate() {
