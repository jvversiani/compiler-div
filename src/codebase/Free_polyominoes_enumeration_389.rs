// Rosetta Code task: Free polyominoes enumeration
// Source: https://rosettacode.org/wiki/Free_polyominoes_enumeration#Rust
...
fn polyominoes_of_rank(number: u32) -> Vec<Polyomino> {
    ...
    let mut result: Vec<Polyomino> = polyominoes.into_iter().collect();
    result.sort();
    result
}
