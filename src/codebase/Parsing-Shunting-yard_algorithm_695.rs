// Rosetta Code task: Parsing/Shunting-yard algorithm
// Source: https://rosettacode.org/wiki/Parsing/Shunting-yard_algorithm#Rust
...
fn main() {
    let input = String::from("(4 * 4) + 4 / 4");

    let result = lex(input.clone())
        .map_err(|c| format!("Invalid character: {}", c))
        .and_then(shunting_yard)
        .and_then(calculate);
