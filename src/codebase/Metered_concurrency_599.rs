// Rosetta Code task: Metered concurrency
// Source: https://rosettacode.org/wiki/Metered_concurrency#Rust
// Content licensed under GFDL 1.2 (Rosetta Code).
// =======================
// Expected output:
// Worker 0 after acquire: count = 3
// Worker 0 after release: count = 4
// Worker 1 after acquire: count = 3
// Worker 1 after release: count = 4
// Worker 2 after acquire: count = 3
// Worker 2 after release: count = 4
// Worker 3 after acquire: count = 3
// Worker 3 after release: count = 4
// Worker 4 after acquire: count = 3
// Worker 4 after release: count = 4
// Worker 5 after acquire: count = 3
// Worker 5 after release: count = 4
// Worker 6 after acquire: count = 3
// Worker 6 after release: count = 4
// Worker 7 after acquire: count = 3
// Worker 7 after release: count = 4
// Worker 8 after acquire: count = 3
// Worker 8 after release: count = 4
// Worker 9 after acquire: count = 3
// Worker 9 after release: count = 4
// =======================
...
fn metered(duration: Duration) {
    ...
    for i in 0..NUM_WORKERS {
        ...
        // Join each worker before spawning the next so the acquire/release
        // ordering (and therefore the printed output) is deterministic.
        .join()
        .unwrap();
    }
    drop(tx);
...
fn main() {
    // Workers run serially, so a tiny hold keeps the demo fast and deterministic.
    metered(Duration::from_millis(1));
}
