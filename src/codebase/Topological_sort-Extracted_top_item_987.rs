// Rosetta Code task: Topological sort/Extracted top item
// Source: https://rosettacode.org/wiki/Topological_sort/Extracted_top_item#Rust
// Content licensed under GFDL 1.2 (Rosetta Code).
// =======================
// Expected output:
// The top levels of the dependency graph are: top1, top2
//
// The compilation order for top level 'top1' is:
// First: [des1a1, des1a2, des1c1, extra1]
//     Then: [des1a, des1b, des1c, ip1a, ip2a, ip2b, ip2c, ipcommon]
//     Then: [des1, ip1, ip2]
//     Then: [top1]
//
// The compilation order for top level 'top2' is:
// First: [des1a1, des1a2, des1c1, extra1]
//     Then: [des1a, des1b, des1c, ip2a, ip2b, ip2c, ipcommon]
//     Then: [des1, ip2, ip3]
//     Then: [top2]
//
// The compilation order for top levels 'top1, top2' is:
// First: [des1a1, des1a2, des1c1, extra1]
//     Then: [des1a, des1b, des1c, ip1a, ip2a, ip2b, ip2c, ipcommon]
//     Then: [des1, ip1, ip2, ip3]
//     Then: [top1, top2]
//
// The compilation order for file 'ip1' is:
// First: [extra1, ip1a, ipcommon]
//     Then: [ip1]
//
// =======================
...
fn print_set<T: std::fmt::Display + Ord + Clone>(a_set: &HashSet<T>) {
    let mut items: Vec<T> = a_set.iter().cloned().collect();
    items.sort();
...
fn main() {
    ...
    // Deterministic iteration order over the top levels.
    let mut sorted_tops: Vec<String> = tops.iter().cloned().collect();
    sorted_tops.sort();

    for top in &sorted_tops {
        println!("The compilation order for top level '{}' is:", top);
        print_compilation_order(&top_extraction(
            data.clone(),
            [top.clone()].iter().cloned().collect(),
        ));
    }
