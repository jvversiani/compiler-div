// Rosetta Code task: P-Adic square roots
// Source: https://rosettacode.org/wiki/P-Adic_square_roots#Rust
...
impl PAdicSquareRoot {
    ...
    fn new(prime: u32, precision: u32, mut numerator: i32, mut denominator: i32) -> Result<Self, String> {
        ...
        let mut order = 0;
        let digits = vec![0; digits_size as usize];
    ...
    fn convert_to_rational(&self) -> Result<String, String> {
        ...
        // Lagrange lattice basis reduction in two dimensions (done in i128 to avoid overflow)
        let mut series_sum: i128 = self.digits[0] as i128;
        let mut maximum_prime: i128 = 1;

        for i in 1..self.precision {
            maximum_prime *= self.prime as i128;
            series_sum += (self.digits[i as usize] as i128) * maximum_prime;
        }

        let mut one: Vec<i128> = vec![maximum_prime, series_sum];
        let mut two: Vec<i128> = vec![0, 1];

        let mut previous_norm: i128 = series_sum * series_sum + 1;
        let mut current_norm: i128 = previous_norm + 1;
        let mut i = 0;
        let mut j = 1;
        ...
        while previous_norm < current_norm {
            ...
            current_norm = ((numerator as f64) / (denominator as f64) + 0.5).floor() as i128;
        ...
        for _ in self.order..0 {
            y *= self.prime as i128;
        }

        for _ in 0..self.order {
            x *= self.prime as i128;
        }
    ...
    fn square_root_odd_prime(&mut self, numerator: i32, denominator: i32) -> Result<(), String> {
        ...
        let mut sum: i128 = first_digit as i128;
        let digits_size = self.precision + 5;

        for i in 2..digits_size {
            let mut next_sum = sum
                - ((coefficient as i128) * ((denominator as i128) * sum * sum - (numerator as i128)));
            next_sum = Self::modulo_i128(next_sum, (self.prime as i128).pow(i));
            next_sum -= sum;
            sum += next_sum;

            let digit = (next_sum / (self.prime as i128).pow(i - 1)) as u32;
    ...
    // i128 version of modulo, for the wide-integer reconstruction paths
    fn modulo_i128(number: i128, modulus: i128) -> i128 {
