// Rosetta Code task: Read a file character by character/UTF8
// Source: https://rosettacode.org/wiki/Read_a_file_character_by_character/UTF8#Rust
...
fn main() -> std::io::Result<()> {
    ...
    // --- Create the file with UTF-8 content ---
    {
        ...
        // ASCII, accented Latin, a 3-byte char, and a 4-byte emoji
        file.write_all("Héllo, wörld!\nUTF-8: αβγ 日本語 😀\n".as_bytes())?;
    } // handle closed here

    // --- Read it back character by character ---
    for (index, value) in ReadUtf8::from(std::fs::File::open(file_path)?).enumerate() {
        match value {
            Ok(c) => print!("{}", c),
