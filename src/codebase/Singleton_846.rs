// Rosetta Code task: Singleton
// Source: https://rosettacode.org/wiki/Singleton#Rust
// Content licensed under GFDL 1.2 (Rosetta Code).
// =======================
// Expected output:
// Initial data: Singleton initialized
// Counter: 1
// Data from another scope: Modified data
// Counter from another scope: 1
...
fn main() {
    ...
    // Spawn and join each thread in turn so the demonstration output is
    // deterministic (thread N always observes counter N + 2).
    for i in 0..3 {
        thread::spawn(move || {
            let singleton = GlobalSingleton::instance();
            let mut instance = singleton.lock().unwrap();
